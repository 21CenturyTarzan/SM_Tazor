  ...
  async getLpTokenAssetNum(networkID: NetworkID, provider: StaticJsonRpcProvider | JsonRpcSigner) {
    ...
    if (true) {
      if (networkID == 56) {
        ...
        tokenNum = Number(reserves[1].toString()) / 10 ** 9; // # of tazor token in lp.
        stableNum = Number(reserves[0].toString()) / 10 ** 18; // # of native token in lp.
        marketCap = (nativePrice * (Number(reserves[0].toString()) * 2)) / 10 ** 18; // usdt decimal is 18 in bsc.
      ...
      if (networkID == 43114) {
        ...
        const reserves = await pairContract.getReserves(); //[0]: tazor, [1] native
        const reserves2 = await stableNativeContract.getReserves(); //[0]: usdt, [1] native
        const nativePrice = Number(reserves2[1].toString()) / Number(reserves2[0].toString());
        tokenNum = Number(reserves[1].toString()) / 10 ** 9; // # of tazor token in lp.
        stableNum = Number(reserves[0].toString()) / 10 ** 18; // # of native token in lp.
        marketCap = (nativePrice * (Number(reserves[0].toString()) * 2)) / 10 ** 18; // usdt decimal is 18 in bsc.
